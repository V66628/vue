      ...
      <div class="py-container">
        ...
        <div class="details clearfix">
            ...
            <div class="navbar-inner filter">
              <ul class="sui-nav">
                <li :class="{active: searchParams.order.indexOf('1')!=-1}" @click="changeOrder('1')">
                  <a>综合 <span v-show="searchParams.order.indexOf('1')!=-1" :class="{'el-icon-bottom':searchParams.order.indexOf('desc')!=-1,'el-icon-top':searchParams.order.indexOf('asc')!=-1}"></span></a>
                </li>
                <li :class="{active: searchParams.order.indexOf('2')!=-1}" @click="changeOrder('2')">
                  <a>价格 <span v-show="searchParams.order.indexOf('2')!=-1" :class="{'el-icon-bottom':searchParams.order.indexOf('desc')!=-1,'el-icon-top':searchParams.order.indexOf('asc')!=-1}"></span></a>
                </li>
              </ul>
            </div>
          </div>
          <div class="goods-list">
            <ul class="yui3-g">
              <li class="yui3-u-1-5" v-for="g in searchList.goodsList" :key='g.id'>
                <div class="list-wrap">
                  <div class="p-img">
                    <a href="item.html" target="_blank"><img :src="g.defaultImg" /></a>
                  </div>
                  <div class="price">
                    <strong>
                      <em>¥</em>
  ...
  export default {
    ...
    methods:{
      ...
      removeKeyword(){
        ...
        this.getdata()
        //通知header组件清除input框里的keyword
        this.$bus.$emit('clear')
        //自己路由跳转它自己，顺便保留query参数
        if(this.$route.query){
      ...
      propsInfo(attr,value){
        const prop=`${attr.attrId}:${value}:${attr.attrValueList}`
      ...
      removeProps(index){
       ...
       changeOrder(flag){
        const {order}=this.searchParams
        const oldFlag=order.split(':')[0]
        const oldsort=order.split(':')[1]
        let newOrder=''
        if(flag==oldFlag){
